...
def bindToMeshBinding(meshVertices, allLABs, voxels, voxelDim):
    ...
    # Vectorize the voxel lookup to find all out-of-bounds points at once
    labs_array = np.array(allLABs)

    # voxels_indices = np.array([
    #     pointToVoxel(lab, voxelDim, x_min, y_min, z_min, max_range)
    #     for lab in labs_array
    # ])
   
    # # Boolean mask of which points are outside the voxel
    # inside = np.array([
    #     bool(voxels[v[0]][v[1]][v[2]]) for v in voxels_indices
    # ])
    ...
    # Query KD-tree for all outside points in one call
